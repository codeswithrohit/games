...
const Login = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
     ...
     const userCredentail= await signInWithEmailAndPassword(firebase.auth(), email, password);
     const userId=userCredentail.user.uid;
      ...
      router.push('/userdashboard.html');
  ...
  return (
    <div className='bg-sky-100 dark:bg-white min-h-screen flex justify-center items-center'>
    <div className="w-full max-w-sm overflow-hidden bg-white rounded-lg shadow-md dark:bg-white">
      <div className="px-6 py-4">
       
  
        <h3 className="mt-3 text-xl font-medium text-center text-blue-500 dark:text-red-300">USER LOGIN</h3>
        ...
        <form onSubmit={handleFormSubmit}>
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <button disabled={loading} className="px-6 text-center py-2 text-sm font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-blue-500 rounded-lg hover:bg-sky-300 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-50">
              {loading ? 'Signing in...' : 'Sign in'}
            </button>
          </div>
         
        </form>
        {/* <p className='mx-4'>Already have an account ?      <a href="/UserSignUp"><button className=''>Signup</button></a>   </p> */}
      </div>
